...
as the successor of ECDSA as it is a more efficient algorithm. Until shortly, in Viacoin and in many other cryptocoins, it
was not possible to implement it without a hardfork. Now that Segwit is implemented, it is.
...
\end{itemize}
\noindent
It supports batch validation, which means that if you have a group of public key
message signature pairs rather than just a single one, you can verify the authenticity of the group as a whole
at a higher speed than each of them individually. This is exactly what we want since blocks are just big batches of signatures to
validate.
\linebreak \noindent
...
\cite{MAST}Mast allowing Viacoin transaction validation scripts to be stored in partially-hashed
...
been completed, saving space because nodes only have to read the top layer of the
Merkle Tree.
\noindent
